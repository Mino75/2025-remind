...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  function updateFixedDisplay() {
    ...
    if (m && d && h !== "" && mi !== "") {
      fixedDisplayDiv.textContent = `Fixed: ${mi} ${h} on ${d}/${m} every year`;
    } else {
  ...
  // A naïve next-occurrence computation from a cron string (for custom frequency)
...
// format coundown
  ...
  function renderReminders(reminders) {
    ...
    const eventDateString = eventDate.toLocaleString();      
      ...
      tr.innerHTML = `
        <td class="countdown" data-duetime="${reminder.dueTime}">${formatCountdown(reminder.dueTime - Date.now())}</td>
...
// set intervall 
  ...
  setInterval(() => {
    document.querySelectorAll('.countdown').forEach(cell => {
      const dueTime = parseInt(cell.getAttribute('data-duetime'));
  ...
  window.editReminder = async (uuid) => {
    ...
    if (reminder) {
      ...
      if (reminder.type === 'frequency') {
        frequencySection.style.display = 'block';
        reminderDueDateField.style.display = 'none';
        // Here, attempt to parse the stored frequency string.
        // If it contains "Fixed:" assume fixed mode; otherwise assume cron.
        if (reminder.frequency.indexOf("Fixed:") !== -1) {
          ...
          if (fixedPart) {
            ...
            // parts[3] should be day/month, e.g., "15/5"
          ...
          fixedFrequencyDiv.style.display = 'none';
          cronBuilderDiv.style.display = 'block';
          // Parse stored cron string from format "X min (Cron: cronString)"
